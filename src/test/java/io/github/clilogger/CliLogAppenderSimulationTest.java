...
/**
 * Visual integration test: wires SLF4J → Logback → CliLogAppender and simulates
 * a realistic mix of slow INFO progress updates, WARN alerts, and ERROR events.
 *
 * Run with: ./gradlew test --tests "*.CliLogAppenderSimulationTest" -i
 * (redirect stdout to the terminal so ANSI rendering is visible)
 */
class CliLogAppenderSimulationTest {

    private static final Logger log = LoggerFactory.getLogger(CliLogAppenderSimulationTest.class);
